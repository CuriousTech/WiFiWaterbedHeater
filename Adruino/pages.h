const char page1[] PROGMEM =
   ...
   "  step=c2.width/cnt\n"
   "\n"
   "  ctx.fillText(12, c2.width/2, 10)//hrs\n"
   "  ctx.fillText(6, c2.width/4, 10)\n"
   "  ctx.fillText(18, c2.width/2+c2.width/4, 10)\n"
   "\n"
   "  ctx.strokeStyle = \"#111\"\n"
   "  if(avg)\n"
   "  {\n"
   ...
   "  }\n"
   "  x = s2x((new Date()).toTimeString())\n"
   "  ctx.beginPath()\n"
   "  ctx.moveTo(x, t2y(waterTemp)) // current time (red)\n"
   "  ctx.lineTo(x, c2.height)\n"
   "  ctx.strokeStyle = \"#F11\"\n"
   "  ctx.stroke()\n"
   "} catch(err) {}\n"
   ...
   "<tr><td colspan=4><canvas id=\"canva\" width=\"300\" height=\"100\" style=\"border:1px dotted;float:left\" onclick=\"draw()\"></canvas></td></tr>\n"
